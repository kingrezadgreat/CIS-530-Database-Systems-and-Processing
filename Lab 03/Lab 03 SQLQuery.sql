...
where e.Fname = 'Reza' and d.Essn=e.Ssn and w.Essn = e.ssn;
...
where e.Sex = 'F' and d.Mgr_ssn=e.Ssn;
...
WHERE E.Dno = D.Dnumber AND W.Essn = E.Ssn) /*as employee */
UNION
(SELECT W.Pno
FROM DEPARTMENT D, WORKS_ON W
WHERE w.essn = D.mgr_ssn) /*as manager */
...
(select d.Dname, d.Dnumber, e.Fname, e.Lname, s.Fname, s.Lname
from DEPARTMENT d, EMPLOYEE e, EMPLOYEE s
where d.Dnumber = e.Dno and e.Super_ssn = s.Ssn)
UNION
(select d.Dname, d.Dnumber, e.Fname, e.Lname, null, null
from DEPARTMENT d, EMPLOYEE e
where d.Dnumber = e.Dno and e.Super_ssn is null )
...
from EMPLOYEE e, DEPARTMENT d
where d.Mgr_ssn=e.Ssn AND Not Exists (select D.essn from Dependent D  where E.ssn = D.essn);
